...
class RPMoveBasePy(RPActionInterface):
    ...
    def saveData(self,cube):
        f = open("~/rosplan_ws/bagfiles/data.txt", 'w')
        pose = PoseStamped()
    ...
    def pose_save(self,models,action,state_check):
        f = open('~/rosplan_ws/src/rosplan_demos/rosplan_demos_interfaces/rosplan_interface_movebase/scripts/position.txt','a')
        get_state_service = rospy.ServiceProxy('/gazebo/get_model_state',GetModelState)
        model = GetModelStateRequest() 
	f.write('--------------------')
	f.write(action+':'+'\n')
	f.write(str(state_check)+'\n')
        for i in models:
            model.model_name = i
    ...
    def concreteCallback(self, msg):
        position = 100
        filr_path1='~/rosplan_ws/bagfiles/baseline/'+str(position)
	...
	print(msg)
	    ...
	    move_base.goto(x1, y1,0.0)
            ...
            print(state_check)
	    self.pose_save({'demo_cube'},which,state_check)		
	    ...
	    p1_move1 = subprocess.Popen('rostopic echo /joint_states > '+path1,close_fds=True,preexec_fn = os.setsid,shell = True)
            ...
            while not rospy.is_shutdown():
                ...
                file1 = open("~/rosplan_ws/bagfiles/cube.json","wb")
                pickle.dump(cube,file1)
                file1.close
                file2 = open("~/rosplan_ws/bagfiles/result.json","wb")
	    ...
	    self.pose_save({'demo_cube'},which,state_check)
            ...
            torso_action.move_to([0.4, ])
            # Place the block
            file1 = open("~/rosplan_ws/bagfiles/cube.json","r")
            cube = pickle.load(file1)
            file1.close
            file2 = open("~/rosplan_ws/bagfiles/result.json","r")
